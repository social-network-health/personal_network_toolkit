...
- [`prm/`](prm/) — Personal Relationship Manager PNA; never-distributed single-user native-SQLite flavor (`never-distributed-single-user` + `native-sqlite-via-filesystem` + `multi-source-merge-with-dedup` + `vanilla-js-spa` + `none` + `shared-only`). First multi-source-dedup demonstrator (`AC-PRM-B`) and the build-from-verifiable-source distribution case. Accepted 2026-06-10; re-archived at `prm@pnt-ref-0.1.1` / `1551896` (prm #59, the `AC-PRM-H` loopback-surface demonstrator) (`swh:1:dir:39637a1ff96f77e7df47e87ac53a7c6d8fc61b62`).
