...
*Design note / spec stub · 2026-06 · status: **proposed (RFC), demonstrated-by: PRM ([PR #59](https://github.com/richbodo/prm/pull/59))**. Complements the [countermeasure library](../../spec/exceptions.md#countermeasure-library) (which already covers the *environmental*, same-UID rows) and the [data-floor](2026-06-data-floor-disclosure-tiers.md) (which bounds the *cloud* surface). Indexed from [`../PriorArt.md` § Design notes](../PriorArt.md).*

> **This is a stub, not a normative change.** It proposes one new flavor-derived AC (`AC-PRM-H`) and a sibling lint, but deliberately does **not** add them to the live AC tables or the lint suite — doing so would assert an obligation before its demonstrator is accepted (and could make a design retroactively non-conformant). Per [`../../CONTRIBUTING.md`](../../CONTRIBUTING.md) § Contribution types, an obligation-imposing AC lands **with the reference design that demonstrates it**. The demonstrator is **PRM**, at [PR #59](https://github.com/richbodo/prm/pull/59) (the daemon session-auth + the loopback-surface lint — green + attested); this note is the design on record so the demonstrator has a target.
