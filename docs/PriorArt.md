...
### 2026-06 — EAR's defensible kernel relocates from at-rest to *encrypt-in-transit* (fellows#256 → toolkit#41)

A follow-on to the at-rest scope decision above. `fellows_local_db` worked the encryption-at-rest question to a decision (fellows#256) and isolated the one defensible target: *not* the live private store (dominated by device FDE, and a `.locked` file contradicts `CST-PWA-SANDBOX-SEALED`'s tool-readability — the same reasoning behind the PR #19 / Ideal-#6 decline), but the **portable export/backup once it leaves the device**. Encrypting *that* artifact seals the in-transit copy without breaking live-store openness. Toolkit effect: a **non-normative frontier note** on `CST-PWA-PRIVATE-SNAPSHOT` and `CST-PWA-NO-SYNC` reframing the long-named `encrypt-then-export-to-self` candidate as **encrypt-in-transit** — confirming, not extending, the at-rest decline; **no new AC**. The pattern's demonstrator is fellows#257 (encrypted portable export); until it ships the note stays a frontier *candidate*, not a `Solved`/`Mitigated` workaround — over-claiming a workaround that doesn't yet exist in code would itself be the false-durability failure the Frontier column exists to prevent.
