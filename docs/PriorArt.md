...
Studied the sibling repo `../cli-printing-press` — a generator that turns an API/website name into a shipped Go CLI + MCP server + skill — as a model for PNA's **build and validation flows**. The core finding: the two projects share one conviction (*a behavior counts only when its conformance can be mechanically checked, not asserted*) but CPP **operationalizes** it far harder — it *runs* the artifact's evidence in a layered pipeline (cheap deterministic gates first/blocking, LLM review second/advisory), where PNA's checking is mostly static lints + LLM reading + trust-the-attestation. PNA's own `SKILL.md` names the gap: a static lint proves a test *exists*, never that it *passes*. Seven recommendations (R1–R7), each *CPP mechanism → PNA gap → fix*, ranked by leverage: **R1** build the behavioral conformance harness (run the designs — this *is* the conformance plan's inert Phase 4); **R2** grow the per-AC deterministic-probe floor; **R3** a per-Goal coverage *vector* (not a grade — honors "checked, not awarded"); **R4** a generated `registry.json` + staged web catalog; **R5** mechanize the contribute-flow gates (re-validate at the pinned commit; auto-fill the PR body); **R6** a per-run "toolkit retro" generalizing `/capture-lesson`; **R7** golden `evaluate-report`s as a posture-drift regression gate. Explicitly *not* copied: a letter grade/badge, a "PNA Press" auto-generator, third-party tooling, market narrowing. Each recommendation routed to the doc that owns its category; the conformance-relevant subset (R1, R7, R5's gate, R2's coverage map) is the follow-on into [`conformance-suite-plan.md`](../plans/conformance-suite-plan.md).
...
### 2026-06 — Loopback-surface auth: the app's own transport is an intrinsic countermeasure (proposal → PRM)

*Full stub: [`design-notes/2026-06-loopback-surface-auth.md`](design-notes/2026-06-loopback-surface-auth.md). Complements the [countermeasure library](../spec/exceptions.md#countermeasure-library) (the environmental same-UID rows) and the [data-floor](design-notes/2026-06-data-floor-disclosure-tiers.md) (the cloud surface); demonstrator: PRM [#59](https://github.com/social-network-health/prm/pull/59).*

The countermeasure library routes "a same-UID agent reads the store" to the advisory **Harden** flow (access-control, outside the app's reach). But a `native-sqlite + vanilla-js-spa` PNA opens its *own* loopback HTTP daemon over the Private DB — and if it is unauthenticated, **any local process can read everything by dialing `127.0.0.1`**, a reach beyond the OS file-permission baseline (a sandboxed / lower-priv / network-only process that can't read the store file can still `curl` the API) and one the app **can** close. That makes it **PNA-intrinsic** — the gap between Surface 2 (cloud MCP: consent + data-floor) and Surface 3 (file reads: Harden). Proposes **`AC-PRM-H`**: generalize [`AC-2`](../spec/axes.md#ac-2) ("server is a delivery channel, not a service") from the `web-bundle` server to the local daemon — loopback-bound + session-authenticated — plus a **loopback-surface lint** (static, sibling of `egress-lint`), and a clarification that the data-floor's `private-sealed` bounds the **cloud** surface only (not Surface 1/3). Authenticating the transport relaxes no guarantee, so it does **not** flip `pna-active` (the not-a-PNA signal stays for Surface 2) — the over-classification the existential review warns against. Demonstrable now: PRM #59 ships the auth + the lint, green + attested. Per CONTRIBUTING it lands **with** PRM; this entry records the design on the stick, not a normative change.

### 2026-06 — Should the spec allow Exceptions at all? Existential review → keep, with three sharper lines

*Full deliberation: [`design-notes/2026-06-exceptions-existential-review.md`](design-notes/2026-06-exceptions-existential-review.md). Proposed changes: [PR #32](https://github.com/social-network-health/personal_network_toolkit/pull/32).*
...
- **Ideal #6, re-tested (2026-06-14) → *conditional* at-rest encryption also declined.** The 2026-06-14 direction session re-opened the at-rest question for one specific threat — an OS-automation AI agent or a rogue local process reading the store — and asked whether *conditional* (rather than blanket) encryption could defend the private layer. Three variants were weighed, from Rich's ideas-PDF and captured in [`brainstorms/2026-06-14-pnt-direction-grill.md`](../brainstorms/2026-06-14-pnt-direction-grill.md) (Cluster 3, ideas b–d): **(b)** encryption scoped to "MCP-on" windows (private withheld/encrypted only while an MCP server is live); **(c)** a per-field *"always encrypt when MCP-accessed"* user preference; **(d)** hazard-triggered encryption (on detecting another process reading the DB, encrypt / sandbox the SQLite behind user acceptance). All three were **declined** — re-confirmed by the data-protection-vs-OS-automation research (R3) that seeded the [countermeasure library](../spec/exceptions.md#countermeasure-library):
  - **The core reason, sharper here than for blanket EAR:** every variant fires at the **live-running** moment, which is exactly when app-level encryption is weakest — the key and plaintext are in memory while the app is active, so a process reading *then* (the very threat these target) sees cleartext. The encryption barely touches the scenario it is aimed at.
  - **Per-field (c)** is dominated by the **[data-floor](design-notes/2026-06-data-floor-disclosure-tiers.md)**: keeping the sensitive fields off the AI-reachable surface *by construction* delivers the same "the AI can't get them" outcome with no keys and without fragmenting tool-readability (`CST-PWA-SANDBOX-SEALED`) field-by-field. (This is the "structural absence vs. encryption as the bounding primitive" question — resolved toward structural.)
  - **Hazard-triggered (d)** is reactive — by the time the unexpected read is detected it may already be done — so its honest assurance class is at best `recoverable-only`, the exact role the **honeytoken + watchdog ("claw trap")** countermeasure already fills; *prevention* is better served by sandboxing the agent or mediating the access path.

  Net: the protective intent is genuine, but app-level conditional encryption is the wrong instrument for it; the countermeasure library routes it to access-mediation + detect-then-respond, which preserve tool-readability. *(Researched since (2026-06): an **OS-level sandbox** that encrypts at the platform layer while keeping the data tool-readable **inside** the sandbox **does** work as an **environmental** countermeasure, and is **not** a revival of app-level EAR — the platform (not the app) holds the at-rest key, and the data stays openable with ordinary tools inside. The sharper finding: the at-rest leg is the easy part — keeping a **same-user** automation agent out is an **access-control problem, not an encryption one**, for which a **separate OS user** (or a VM) is the convenient desktop answer, and which **mobile** platforms get for free via per-app UIDs. Detail in [`exceptions.md` § Countermeasure library](../spec/exceptions.md#countermeasure-library), "On confining a same-UID agent.")*
