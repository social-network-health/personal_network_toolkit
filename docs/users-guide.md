...
### Audit an adjacent app (a goal-impact read)

Not every app worth auditing claims to be a PNA. Your messenger, your mail client, your CRM — anything that stores contact data or private relationship data (notes, groups, sensitive communications) — can be read against the same four Goals, and the answer is often exactly what you want to know: *how does this app treat the people-data I keep in it?* This is a **routine** use of the audit flow, not a special case; the modes are defined in [`docs/conformance-scope-and-lifecycle.md` § The utility line](conformance-scope-and-lifecycle.md).

1. **Run the same audit** ([steps above](#audit-a-candidate-pna-before-installing-it)) against the app's source:

   > "Use the toolkit skill to evaluate `<path>` against the PNA spec — how does it treat my contact and relationship data?"

2. **The agent classifies it and switches modes.** Because the app doesn't claim to be a PNA, the report's verdict is not a pass/fail on membership — its posture reads **`not-a-pna`**, and the verdict is a **per-Goal impact read**: `protects` / `neutral` / `diminishes` / `mixed` / `out-of-scope` for each Goal, each `diminishes` or `mixed` grounded in the AC findings behind it. The AC walk still runs underneath — that's what makes the read precise and diffable — but a "non-conformant" entry there is a class-contrast observation, not a failure of something the app promised. The high-value signal is `diminishes`: the app erodes your control of your personal-network root.
...
| See how an adjacent app treats your people-data | PNA Toolkit (with the app's source at known path) | "Use the toolkit skill to evaluate `<path>` against the PNA spec — how does it treat my contact and relationship data?" |
