---
name: pna-toolkit
description: 'Use when building, extending, or evaluating a Personal Network Application (PNA) — local-first, private-by-default applications that mirror SaaS contact data into a user-owned workspace and operate on relationship data with no remote authority. Triggers on requests to build a local-first contact app, relationship manager, private CRM-like tool, or any app built around personal-network data; on requests to audit an existing application for PNA-spec conformance ("is this app safe to install?", "does my app conform?") — including ADJACENT apps that never claimed to be PNAs but store contact or relationship data (a messenger like Signal, a mail client, a CRM: "how does this app treat my contacts and private notes?" gets a per-Goal goal-impact read, not a membership verdict); on requests to contribute back to the PNA Toolkit — either a reference design, or (the common case) a lighter toolkit fix to the spec/tooling/docs; and on requests to harden the operating environment a PNA runs in against runtime adversaries (an OS-level AI agent or another local process reaching the data out-of-band). Four flows: build a conformant PNA from the spec, evaluate whether an existing application conforms (or how an adjacent app relates to the PNA Goals), contribute back to the toolkit (reference design or toolkit fix), and harden the environment a PNA runs in (advisory).'
---
...
Use when the user wants to audit an application for PNA conformance — someone else's candidate ("is this app safe to install?"), their own in-progress design ("does my app conform yet?"), or an **adjacent app** that never claimed to be a PNA but touches contact/relationship data ("how does Signal treat my private notes?"). Mechanics are the same in all cases; the classification step below decides what kind of verdict the report may honestly take, and the user's framing tells you which ACs or Goals to emphasize.

Inputs: a candidate's source tree (or a description sufficient to read its behavior), and either a declared set of axis picks (from an Architecture document, if present) or picks you can infer from the source.

1. **Classify the candidate first — choose the output mode.** Before judging any AC, establish: what **class** of application is this (its own claims plus its architectural shape)? Does it store **contact data**? Does it store **private relationship data** (notes, tags, groups, comms history, sensitive communications — the [`spec/PNA_Spec.md` § Vocabulary](../spec/PNA_Spec.md#vocabulary) senses)? That nexus with the personal-network root selects the output mode — the three-mode line owned by [`docs/conformance-scope-and-lifecycle.md` § The utility line](../docs/conformance-scope-and-lifecycle.md):
   - **Mode 1 — membership.** The candidate claims to be a PNA (an Architecture document, an explicit statement) or is architecturally shaped like one. Run the full flow below; the verdict is a conformance posture.
   - **Mode 2 — goal-impact.** It doesn't claim to be a PNA, but it stores contact and/or relationship data (a messenger, a mail client, a CRM). Run the same AC walk below as the **instrument** — the AC findings are what make the read precise and diffable — but the **verdict** is the per-Goal read (`protects` / `neutral` / `diminishes` / `mixed` / `out-of-scope`, one entry per Goal) and the posture is `not-a-pna`, never a membership verdict the app didn't claim. A `non-conformant` AC finding in this mode is a class-contrast observation feeding a Goal read, not the failure of a claim; say so in its rationale.
   - **Mode 3 — out of scope.** No apparent nexus (a photo editor, a game). Do **not** silently refuse: ask the user how they use the app — a declaration like *"it's an editor, but I keep all my contacts in it and mail people from it"* establishes nexus, is recorded **verbatim** in the report (`nexus_source: user-declared`), and the evaluation proceeds in Mode 2. With no declaration, the honest output is one line — *out of scope: the toolkit has nothing useful to say here* — and no report is emitted.

   The runner may force Mode 1 on an adjacent app ("evaluate it as a candidate PNA anyway"); record the nexus honestly either way. Whatever the mode, the classification — application class, the two storage answers, nexus, nexus source, mode — is recorded in the report's `candidate.classification` block as a matter of course: a reader of any report learns first *what kind of app this is*.

2. **For each AC in `spec/PNA_Spec.md` that applies to the candidate's flavor**, decide conformance:
...
3. **For each conditional AC in `spec/PNA_Spec.md` § Conditional architectural commitments** triggered by the candidate's axis picks, do the same.
4. **For each typed contract relevant to the candidate's axis picks**, check that the candidate implements the contract correctly. Contract headers (`Realizes: AC-X, AC-Y`) tell you which ACs the contract serves.
5. **Attestation evidence audit — the Security Target is only as good as its executable evidence.** For each AC/CST the candidate attests `conformant`:
...
6. **Detect and verify Exceptions** (see `spec/exceptions.md`). For each Exception the candidate can raise — declared in its Architecture document's exception attestation, or inferred from the source where undeclared:
...
7. **Detect and verify Constraints** (see `spec/constraints.md`). For each Constraint the candidate's axis picks inherit:
...
8. **Detect and verify User-mediation** (see `spec/user_mediation.md`). Enumerate the candidate's **mediated boundaries** — every distinct path that mutates the Private store or egresses its data (private-data restore, group/tag/note edits, outreach, AI-proposed merges) — and at each verify the three properties, reporting per boundary by `UM-*`:
...
10. **Summarize at the top** (the artifact's `summary` object): overall posture and the most concerning non-conformances. Goals 1–4 are the load-bearing user-facing concerns — anything compromising ownership of the root (Goal 1), integrity-by-validation (Goal 2 — which absorbs sourced-data honesty and local diagnosability), protection from egress / private-data sovereignty / user-controlled communication (Goal 3), or durability against entropy and accidents (Goal 4) leads the summary. The `posture` reports **PNA membership** (the `pna-active` bit, `exceptions.md` § Concept): an app with an active exception reads **`not-pna-active`** (never `conformant`) — how honestly each exception is handled stays in the per-`EX-*` findings, never rolled into the top-line verdict. **In Mode 2 the posture is `not-a-pna`** and the verdict lives in `summary.goal_impacts` — one entry per Goal on the fixed scale, each `diminishes` or `mixed` read carrying a note that names the AC findings it leans on; the headline states the application class first ("X is a Y, not a PNA — evaluated as a goal-impact read"), then the strongest reads in both directions. The scale is bounded strictly to the Goals — never a general privacy or quality score (that boundary is what keeps the read meaningful; `docs/conformance-scope-and-lifecycle.md` § The utility line).
